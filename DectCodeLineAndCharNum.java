...
//	如果选择了emptyLineMode==true模式，则不计入。
	...
	public static String adds = "src/code";//文件夹路径，程序将计算该路径下所有代码文件的行数和字符数
	public static String extensions=".py.java.h.cpp.md";//合法的文件后缀
...
//		检测一个文件的字符数
		...
		String[] tails=ad.split("\\.");
		String tail=tails[tails.length-1];
		
		if(!extensions.contains(tail)){
			excludeSign=true;
			return 0;
		}
		BufferedReader in = new BufferedReader(new InputStreamReader(
...
//		检测一个文件夹内的代码行数和字符数
		...
		if(!path.isDirectory()){
			...
			if(excludeSign==false){
				...
				charNum+=num;
				lineNum+=num2;
				System.out.println(filesNum+"  :  "+ad+"  : "+num2+" 行 ，   "+num+"  个字符.");				
			}
			return;
		}
		String prenames[] = path.list();// 文件名
...
//			System.out.println(filenames.get(i));
			...
			if(excludeSign==false){
				...
				charNum+=num;
				lineNum+=num2;
				System.out.println(filesNum+"  :  "+filenames.get(nos.get(i))+"  : "+num2+" 行 ，   "+num+"  个字符.");				
			}
			int pos=nos.get(i);
			filenames.remove(pos);
		}
...
//		return fileCharNum;
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("\n文件数： "+filesNum+"   ,代码总行数： "+lineNum+"  ,字符总数： "+charNum);
